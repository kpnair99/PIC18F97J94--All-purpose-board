...
#define RTC_WRITE_ADDR (RTC_SLAVE_ADDR << 1)   // 0xDE
#define RTC_READ_ADDR  ((RTC_SLAVE_ADDR << 1) | 1)  // 0xDF
...
void RTC_SetTime(uint8_t sec) {
    I2C_start();
    delay_ms_(1);
    I2C_write_byte(RTC_WRITE_ADDR);
    delay_ms_(1);
...
//    
//}
unsigned char yp=0x0;
void RTC_ReadTimeSequential(uint8_t *buffer, uint8_t length) {
    I2C_start();
    delay_ms_(2);
    I2C_write_byte(0xDF);
    delay_ms_(2);
...
//        
        ...
        delay_ms_(50);
        RTC_ReadTimeSequential(buffer2, 1);
        delay_ms_(20);
    }
}
