...
#include <xc.h>
#define _XTAL_FREQ 8000000UL  // Adjust as per your oscillator
#include <stdint.h>
#include <pic18f97j94.h>

// === Chip Select Control ===
#define DAC_CS           LATCbits.LATC2
#define DAC_CS_DIR       TRISCbits.TRISC2

// === MCP4822 Command Bits ===
// Channel A = 0, Channel B = 1
// Buffer: 1 = Buffered VREF
// Gain: 1 = 1x (full-scale), 0 = 2x
// Shutdown: 1 = Active, 0 = Shutdown
#define DAC_CMD 0x3FFF//0x3C7A
//uint8_t voltage;
void SPI_CS_Low() {
    DAC_CS = 0;
    __delay_us(1);
...
void SPI2_PinInit(void);
void SPI2_Init(void);
void SPI2_Write(uint8_t data);
void DAC_Write(uint16_t value12); // Writes to Channel A

// === Configure PPS and CS pin ===
...
// === Write 1 byte via SPI2 ===

void SPI2_Write(uint8_t data) {
    SSP2BUF = data;
    while (!SSP2STATbits.BF); // Wait for transmit complete
}

// === Write 12-bit value to MCP4822, Channel A ===

void DAC_Write(uint16_t value12) {
    uint16_t cmd = DAC_CMD; // Ch A, buffered, 1x, active

    SPI_CS_Low(); // Start SPI frame
    SPI2_Write(cmd >> 8); // Send MSB
    SPI2_Write(cmd & 0xFF); // Send LSB
    SPI_CS_High(); // End SPI frame
...
void main(void) {
    ...
    while (1) {
        //        DAC_Write(0);       // 0V
        //        __delay_ms(1000);
        //        DAC_Write(2048);    // Mid-scale
        //        __delay_ms(1000);
       DAC_Write(DAC_CMD); // Full-scale
        __delay_ms(1000);
    }
}